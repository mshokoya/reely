...
function envCheck() {
  if (!process.env.OTEL_TRACE_URL) {
    throw new Error(
      "[ERROR][setProvider]: process.env.OTEL_TRACE_URL is not defined",
    );
  }
  if (!process.env.OTEL_METRIC_URL) {
    throw new Error(
      "[ERROR][setProvider]: process.env.OTEL_METRIC_URL is not defined",
    );
  }
  if (!process.env.APP_NAME) {
    throw new Error(
      "[ERROR][setProvider]: process.env.APP_NAME is not defined",
    );
  }
  if (!process.env.APP_VERSION) {
    throw new Error(
      "[ERROR][setProvider]: process.env.APP_VERSION is not defined",
    );
  }
}
