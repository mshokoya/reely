...
export const ManagerApplications = observer(() => {
  const activeTab = useObservable("all");
  const applications = managerApplicationRoute.useLoaderData() as Application[];
  ...
  const filteredApplications = applications?.filter((application: Application) => {
    if (activeTab.get() === "all") return true;
    return application.status.toLowerCase() === activeTab.get();
  });
                ...
                >
                  <div className="flex justify-between gap-5 w-full pb-4 px-4">
                    ...
                    <div className="flex gap-2">
                      <Link
                        to={`/managers/properties/${application.property.id}`}
